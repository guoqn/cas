...
(function($) {
	...
	function MessageDialog() {
			...
			'<div data-backdrop="false" data-keyboard="false" class="modal fade message-dialog" tabindex="-1" role="dialog">' +
			  '<div class="modal-dialog">' +
			    '<div class="modal-content">' +
			      '<div class="modal-header"><h4 class="modal-title"><span></span><span></span></h4>' +
		...
		this.$dialogconfirm = $(
				'<div data-backdrop="false" data-keyboard="false" class="modal fade message-dialog" tabindex="-1" role="dialogconfirm">' +
				'<div class="modal-dialog">' +
				'<div class="modal-content">' +
	...
	MessageDialog.prototype = {
		...
		closeconfirm: function(callback) {
			this.$dialogconfirm.modal('hide');
			if (typeof callback === 'function') {
				callback();
			}
		},
		...
		openconfirm: function(options) {
			...
			// set content
			this.$dialogconfirm.children().removeClass().addClass('modal-dialog').addClass(DIALOG_SIZES[options.size]).addClass(options.style);
			this.$dialogconfirm.find('.modal-title > span:first-child').removeClass().addClass(options.icon);
			this.$dialogconfirm.find('.modal-title > span:last-child').text(options.title);
			this.$dialogconfirm.find('.modal-body > p').text(options.message);
			
			// create buttons
			var footer = this.$dialogconfirm.find('.modal-footer').empty(); // clear footer
			if ($.isArray(options.buttons) && options.buttons.length > 0) {
				footer.show();
				var that = this;
				$.each(options.buttons, function(index, button) {
					$('<button type="button" class="btn btn-sm"></button>').click(function() {
						that.closeconfirm(button.click);
					}).addClass(button.style || 'btn-default').text(button.label || '按钮').appendTo(footer);
				});
			} else {
				footer.hide();
