...
ffc.context.loginInfo = {};/*登陆信息*/

// 获取配置调用目标CRM2域
$.ajax({
    url: contextPath + "/sys/sysQuery/qryCrm2Url?urlCode=CRM2_URL",
    async: false,
    success: function (data) {
        ffc.context.crm2 = data;
    }
});// 域
//ffc.context.crm2URL = "http://134.132.66.44:8090/crm" + "/mod_ds/ds_invoke_index.zul";
...
commonApp.service("commonService", function ($http) {
    ...
    this.call = function (url, param, sucCallback, errCallBack) {
        return http("POST", url, param).success(function (data, status, headers, config) {
            if (angular.isFunction(sucCallback)) {
                if (data.result == false && undefined != data.exceptions && data.exceptions.length > 0) {
    ...
    this.invoke = function (url, params) {
        if (ffc.util.isAvailable(url)) {
            ...
            MESSAGE_DIALOG.warning("请求访问服务器失败!(地址:" + ffc.context.crm2 + "，请检查)");
        }
    };
})
...
ffc.util.loadTableData = function (render, result) {
    if (result && result.result && undefined != result.pageInfo && undefined != result.pageInfo.list) {
        render.success({
...
ffc.util.loadTableDataList = function (render, result) {
    if (result && result.result && undefined != result.dataList) {
        render.success({
...
ffc.util.setListResult = function ($scope, errorValue, render, result) {
    if (!$scope.errorTips) {
        $scope.errorTips = {};
    }
    if (render) {
        if (result.result) {
            $scope.errorTips[errorValue] = null;
            ffc.util.loadTableDataList(render, result);
        } else {
            $scope.errorTips[errorValue] = ffc.util.isEmpty(result.msgTitle[0]) ? ffc.context.errorTip : result.msgTitle[0];
        }
    } else {
...
if (!ffc.util.isEmpty(localUrl)) {
    ...
    if ("1" === ffc.util.getHrefValue(params, "auto")) {
        ...
        var login = {};
        // 设置登录信息
        login.staffId = ffc.util.getHrefValue(params, "staffId");
        login.orgId = ffc.util.getHrefValue(params, "orgId");
        login.regionCd = ffc.util.getHrefValue(params, "regionCd");
        login.areaId = ffc.util.getHrefValue(params, "areaId");
        login.channelCode = ffc.util.getHrefValue(params, "channelCode");
        login.staffPositionId = ffc.util.getHrefValue(params, "staffPositionId");
        login.isWholeReginPri = ffc.util.getHrefValue(params, "isWholeReginPri");
        // 设置服务器Session

        $.ajax({
            method: "POST",
            async: false,
            url: contextPath + "/sys/loginCtrl/login",
            data: {
                loginInfo: JSON.stringify(login)
            },
            success: function (data) {
                if (!ffc.util.isEmpty(data) && !ffc.util.isEmpty(data.object)){
                    ffc.context.loginInfo = data.object;
                } else {
                    MESSAGE_DIALOG.error("Seesion信息设置失败!"+data.msgTitle);
                }
            }
        });
    }
