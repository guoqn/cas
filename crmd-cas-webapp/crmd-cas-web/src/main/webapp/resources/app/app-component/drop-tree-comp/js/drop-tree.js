...
(function(angular) {
	...
	function createHtml() {
		var _html = "<div class='dropdown'>" +
			...
			"<input readonly type='text' class='form-control dateTime-btn' placeholder='请选择区域' />" +
			"<div class='input-group-addon comp-btn' style='width:30px;cursor:pointer;border-left:none;'><span class='caret'></span></div>" +
			"</div></div>" +
	...
	function dropTreeListener(elm) {
	    var targetElm = elm.find(".dropdown-toggle")
		targetElm.on("click.diy-dropdown", function(e) {
		    e.stopPropagation()
			toggleTreePanel(this);
		});
		$(document).on("click.diy-dropdown", function(e){
		    var clickObj = $(e.target)
            if ((elm.prop('outerHTML').indexOf(clickObj.prop('outerHTML'))) == -1) {
                $(targetElm).attr("aria-expanded", "false").parent().removeClass("open");
                //toggleTreePanel(targetElm);
            }
		})
	}
